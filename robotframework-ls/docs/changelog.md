...
- Linting now flags typed `Secret` variables initialized with literal values to reflect Robot Framework 7.4 restrictions.
- Attribute completions now suggest `.value` for typed `Secret` variables so their underlying values are easier to access.
...
- Added option (`robot.completions.dictionaryEntries.enable`) to enable or disable dictionary key completions.
